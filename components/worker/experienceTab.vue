...
<template>
	<a-form ref="formRef" layout="vertical" name="dynamic_form_nest_item" :model="dynamicValidateForm" @finish="onFinish">
		<a-row :gutter="20" v-for="(exp, index) in dynamicValidateForm.experiences" :key="exp.id">
			...
			<a-col span="12">
				<a-form-item label="Position" :name="['experiences', index, 'position']">
					<a-input v-model:value="exp.position" placeholder="Position" />
				</a-form-item>
			</a-col>
			<a-col span="12">
				<a-form-item label="Start Date" :name="['experiences', index, 'startDate']">
					<a-date-picker v-model:value="exp.startDate" placeholder="Start Date" class="w-full" />
				</a-form-item>
			</a-col>

			<a-col span="12">
				<a-form-item label="End Date" :name="['experiences', index, 'endDate']" v-if="!exp.currentlyWorking">
					<a-date-picker v-model:value="exp.endDate" placeholder="End Date" class="w-full" />
				</a-form-item>
			</a-col>
			<a-col span="24" class="flex justify-between">
				<a-form-item :name="['experiences', index, 'currentlyWorking']">
